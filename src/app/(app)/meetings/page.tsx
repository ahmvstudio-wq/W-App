...
export default function MeetingsPage() {
  ...
  const [activeModalTab, setActiveModalTab] = useState<'summary' | 'transcript' | 'actions' | 'chapters'>('summary')
  ...
  async function fetchMeetings(silent = false) {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.meetings)) {
        ...
        if (selectedMeeting) {
          const updated = data.meetings.find((m: any) => m.id === selectedMeeting.id)
  ...
  async function handleSyncFathom() {
    ...
    try {
      ...
      toast.success('Synced recordings from Fathom AI!')
    } finally {
      setSyncing(false)
    }
  }
  ...
  async function convertActionToTask(item: FathomActionItem, meetingTitle: string) {
    try {
      ...
      const data = await res.json()
      if (data.success) {
        toast.success(`Task created: "${item.text.slice(0, 30)}..."`)
  ...
  const totalActionItems = meetings.reduce((acc, m) => acc + m.action_items.length, 0)
    ...
    <div className="space-y-8 pb-16 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-mono text-[#6b7280] uppercase tracking-wider mb-1 font-light">
            ...
            <span>•</span>
            <span className="text-black font-normal">FATHOM AI VIDEO & MEETINGS</span>
          </div>
          <h1 className="text-3xl font-light tracking-tight text-black flex items-center gap-3">
            <span>Executive Meetings & Transcripts</span>
        ...
        <div className="flex items-center gap-3 font-body">
          <button
            ...
            disabled={syncing}
            className="flex items-center gap-2 px-4 py-2 bg-white hover:bg-[#f5f5f7] border border-black/[0.08] text-black font-normal text-xs rounded-xl shadow-sm transition-all cursor-pointer"
          >
            <RefreshCw size={14} className={syncing ? 'animate-spin' : ''} />
            <span>{syncing ? 'Syncing Fathom...' : 'Sync from Fathom'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 font-body">
        ...
        <div className="p-5 rounded-3xl bg-gradient-to-br from-white via-white to-emerald-50/50 border border-black/[0.06] shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            <span className="text-[10px] font-mono text-[#6b7280] uppercase tracking-wider font-light">Action Items Extracted</span>
            <div className="text-2xl font-light text-emerald-600 tracking-tight">{totalActionItems}</div>
            <div className="text-[11px] text-[#9ca3af] font-mono">Synced with Chief AI</div>
          </div>
        ...
        <div className="p-5 rounded-3xl bg-gradient-to-br from-white via-white to-purple-50/50 border border-black/[0.06] shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            <span className="text-[10px] font-mono text-[#6b7280] uppercase tracking-wider font-light">Key Attendees</span>
            <div className="text-2xl font-light text-purple-700 tracking-tight">{totalAttendees}</div>
            <div className="text-[11px] text-[#9ca3af] font-mono">Team Stakeholders</div>
          </div>
        ...
        <div className="p-5 rounded-3xl bg-gradient-to-br from-white via-white to-amber-50/50 border border-black/[0.06] shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            <span className="text-[10px] font-mono text-[#6b7280] uppercase tracking-wider font-light">Fathom Connection</span>
            ...
            <div className="text-[11px] text-[#9ca3af] font-mono">Webhook Listener Online</div>
          </div>
      ...
      {loading ? (
        ...
        <div className="py-20 text-center rounded-3xl bg-white border border-dashed border-black/[0.1] text-[#6b7280] text-xs font-body font-light">
          No meeting recordings found. Connect a call on Zoom/Google Meet with Fathom to automatically log here.
        </div>
      ) : (
        <div className="space-y-4 font-body">
          {filteredMeetings.map((meeting) => (
            ...
            >
              ...
              {/* Right Column: Action Items Badge & View Button */}
              <div className="flex md:flex-col items-end justify-between gap-3 flex-shrink-0">
                <div className="flex items-center gap-2">
                  <span className="px-3 py-1 bg-[#fafafa] border border-black/[0.06] rounded-xl text-xs font-mono text-[#4b5563] flex items-center gap-1.5">
                ...
                <div className="flex items-center gap-2 text-xs font-normal text-black group-hover:translate-x-1 transition-transform">
                  <span>Open Transcript & Video</span>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 sm:p-8 animate-fadeIn font-sans">
          <div className="bg-white border border-black/[0.08] rounded-3xl w-full max-w-4xl max-h-[90vh] shadow-2xl flex flex-col overflow-hidden font-body">
              ...
              <div className="flex items-center gap-2">
                {selectedMeeting.share_url && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="p-2 text-[#9ca3af] hover:text-black hover:bg-black/[0.04] rounded-xl transition-colors flex items-center gap-1.5 text-xs"
            ...
            <div className="px-6 border-b border-black/[0.06] flex gap-6 text-xs flex-shrink-0 bg-white">
              {[
                { id: 'summary', label: 'AI Summary & Key Takeaways', icon: Sparkles },
                ...
                { id: 'chapters', label: `Chapters (${selectedMeeting.chapters.length})`, icon: Clock },
              ].map((tab) => (
                <button
                  key={tab.id}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#fbfbfd]">
              {activeModalTab === 'summary' && (
                <div className="space-y-6">
                  ...
                  <div className="p-6 rounded-2xl bg-white border border-black/[0.06] shadow-sm space-y-3">
                    <span className="text-[10px] font-mono text-[#6b7280] uppercase tracking-wider block">EXECUTIVE AI SUMMARY</span>
                    <p className="text-xs text-[#374151] leading-relaxed font-light whitespace-pre-wrap">
                  ...
                  {/* Key Takeaways */}
                  <div className="p-6 rounded-2xl bg-white border border-black/[0.06] shadow-sm space-y-3">
                    <span className="text-[10px] font-mono text-[#6b7280] uppercase tracking-wider block">KEY DECISIONS & TAKEAWAYS</span>
                    <ul className="space-y-2">
                      {selectedMeeting.key_takeaways.map((takeaway, idx) => (
                        <li key={idx} className="flex items-start gap-2.5 text-xs text-[#374151] font-light">
                      ...
                      >
                        <div className="space-y-1 flex-1">
                          ...
                          <div className="text-[10px] text-[#9ca3af] font-mono font-light">
                            Assignee: {item.assignee || 'Unassigned'} • Timestamp: {Math.floor((item.timestamp_seconds || 0) / 60)}m
                          </div>
                        </div>
              ...
              {activeModalTab === 'transcript' && (
                <div className="space-y-3">
                  <span className="text-[10px] font-mono text-[#6b7280] uppercase tracking-wider block mb-2">
                    TIMESTAMPED TRANSCRIPT & SPEAKER INTELLIGENCE
                  </span>

                  {selectedMeeting.transcript.length === 0 ? (
                    <div className="p-8 text-center text-xs text-[#9ca3af] bg-white rounded-2xl border border-black/[0.06]">
                      Transcript processing on Fathom AI.
                    </div>
                  ) : (
              ...
              {activeModalTab === 'chapters' && (
                <div className="space-y-3">
                  <span className="text-[10px] font-mono text-[#6b7280] uppercase tracking-wider block mb-2">
                    MEETING CHAPTERS & TOPIC TIMELINE
                  </span>

                  {selectedMeeting.chapters.map((chap, idx) => (
                    <div key={idx} className="p-4 px-5 rounded-2xl bg-white border border-black/[0.06] shadow-sm flex items-start gap-4">
                      <span className="px-2 py-1 bg-black/[0.04] text-black font-mono text-[10px] rounded-lg flex-shrink-0">
                        {chap.timestamp}
                      </span>
                      <div className="space-y-1">
                        <h4 className="text-xs font-normal text-black">{chap.title}</h4>
                        <p className="text-xs text-[#6b7280] font-light leading-relaxed">{chap.summary}</p>
                      </div>
                    </div>
                  ))}
                </div>
